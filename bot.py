...
# Путь к библиотеке в общем хранилище
SHARED_LIB_PATH = '/app/shared/vk_api_lib'

# Добавляем путь в список поиска библиотек
if os.path.exists(SHARED_LIB_PATH):
    sys.path.insert(0, SHARED_LIB_PATH)
    print(f"✅ Библиотека загружена из: {SHARED_LIB_PATH}")
else:
    print(f"❌ Путь {SHARED_LIB_PATH} не найден! Проверьте, распакован ли архив.")

# Теперь импорт должен работать
try:
    import vk_api
    print("✅ vk_api успешно импортирован!")
except ImportError as e:
    print(f"❌ Ошибка импорта vk_api: {e}")
